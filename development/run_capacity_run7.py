"""
CHARLIEC – RUN 7 CAPACITY TEST.
Runs matrix run 7 locally; produces Live Results/Generator Capacity Run 7.txt.
Requires env: SPEC_ID, WORKSPACE_ID.
Execution rules: same fields and semantics as Run 1 result file.
"""
import os
import sys
import json
...
# Allow up to 1800s wall-clock to observe completion behaviour (as per Runs 4–8 update),
# but the result file format and status fields follow Run 1.
TIMEOUT_SEC = 1800
SINGLE_RUN_SCRIPT = REPO_ROOT / "development" / "run_single_capacity_run.py"
OUTPUT_FILE = REPO_ROOT / "Live Results" / "Generator Capacity Run 7.txt"
...
def main():
    ...
    try:
        ...
        if not out:
            r = {
                ...
                if proc.returncode and "timeout" in str(proc.stderr or "").lower()
                else "failure",
                "run_id": None,
    ...
    # Build deliverable per CHARLIEC RUN 7 spec (same fields as Run 1)
    lines = [
        "CHARLIEC – RUN 7 CAPACITY TEST",
        "===============================",
        "",
