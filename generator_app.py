...
if page == "📊 Dashboard":
    ...
    for spec in specifications:
        ...
        with st.expander(f"📰 {spec.get('newsletter_name', 'Unnamed')} - {spec.get('frequency', '').title()}"):
            ...
            with col1:
                ...
                if last_run:
                    st.write("**Last Generated:**", format_ts_local(last_run["created_at"]))
                else:
            ...
            with col2:
                ...
                if is_allowed:
                    ...
                    if next_date:
                        st.write("**Next Eligible:**", format_ts_local(next_date.isoformat()))
    ...
    with col1:
        ...
        st.write("**Created:**", format_ts_local(spec.get("created_at") or ""))
    ...
    if not is_allowed:
        ...
        if next_date:
            st.info(f"⏰ Next eligible generation date: **{format_ts_local(next_date.isoformat())}**")
        st.stop()
    ...
    if not history:
        ...
        for run in history:
            ...
            with st.expander(f"{'✅' if status == 'success' else '❌' if status == 'failed' else '⏳'} {format_ts_local(run.get('created_at') or '')} - {status.upper()}"):
                col1, col2 = st.columns(2)
